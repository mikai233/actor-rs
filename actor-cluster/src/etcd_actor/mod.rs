...
impl EtcdActor {
    pub fn new(context: &mut ActorContext, client: EtcdClient) -> Self {
        let keep_alive_resp_waker = futures::task::waker(Arc::new(PollKeepAliveRespWaker {
            actor: context.myself().clone(),
        }));
        let watch_resp_waker = futures::task::waker(Arc::new(PollWatchRespWaker {
            actor: context.myself().clone(),
        }));
        Self {
            client,
    ...
    fn keep_alive_failed(id: i64, applicant: &ActorRef, error: Option<etcd_client::Error>) {
        let keep_alive_failed = KeepAliveFailed { id, error };
        applicant.tell(DynMessage::orphan(keep_alive_failed), ActorRef::no_sender());
    }
}
