...
impl<A> Debug for UserDelegate<A>
where
    A: Actor,
{
    ...
    A: Actor,
{
    ...
    pub fn downcast<M>(self) -> anyhow::Result<M>
    where
        M: Message,
    {
...
impl<A> CodecMessage for UserDelegate<A>
where
    A: 'static + Actor + Send,
{
    ...
    fn into_dyn(self) -> DynMessage {
        ...
        DynMessage {
            name,
            ty: MessageType::User,
            message: message.into_codec(),
        }
    }
}

#[async_trait]
impl<A> Message for UserDelegate<A>
where
    A: Actor + Send + 'static,
{
    type A = A;
