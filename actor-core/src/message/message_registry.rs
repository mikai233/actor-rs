...
impl MessageRegistry {
    ...
    pub fn register<M>(&mut self, id: u32)
    where
        M: CodecMessage,
    {
        let name = type_name::<M>();
        let decoder = M::decoder().unwrap_or_else(|| panic!("{} decoder is empty", name));
        assert!(
    ...
    pub fn register_user<M>(&mut self)
    where
        M: CodecMessage,
    {
    ...
    pub fn register_system<M>(&mut self)
    where
        M: CodecMessage,
    {
    ...
    ) -> anyhow::Result<IDPacket> {
        let id = *self
            .id
            .get(name)
            .ok_or(anyhow!("message {} is not registered", name))?;
        let bytes = message.encode(self)?;
        let packet = IDPacket { id, bytes };
        Ok(packet)
    }
    ...
    pub fn decode(&self, packet: IDPacket) -> anyhow::Result<DynMessage> {
        ...
        let decoder = self
            .decoder
            .get(&id)
            .ok_or(anyhow!("message with id {} is not registered", id))?;
