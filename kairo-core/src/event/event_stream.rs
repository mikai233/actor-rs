...
impl EventStream {
        ...
        T: Fn(E) -> DynMessage + Send + Sync + 'static,
    {
        ...
        let transform =
            move |message: DynMessage| match message.into_inner().into_any().downcast::<E>() {
                Ok(message) => Some(transform(*message)),
                Err(_) => None,
            };
        ...
        if let Some(subscribers) = self.subscriptions.get(event_name) {
            subscribers.iter().for_each(move |s| {
                ...
                match msg {
                    None => {
                        warn!(
                            "event {} cannot send to {}, transform or message incorrect",
                            event_name, s.subscriber
                        );
                    }
                    Some(msg) => {
...
mod event_tests {
    ...
    use kairo_derive::{COrphanEmptyCodec, EmptyCodec};
