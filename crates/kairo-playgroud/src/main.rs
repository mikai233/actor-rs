...
use kairo_core::{DynMessage, EmptyTestActor};
use kairo_playgroud::common::build_cluster_setting;
use kairo_playgroud::common::test_message::TestMessage;
...
async fn main() -> anyhow::Result<()> {
    ...
    let system1 = ActorSystem::new(
        "mikai233",
        build_cluster_setting("127.0.0.1:12121".parse()?, client.clone())?,
    )?;
    let system2 = ActorSystem::new(
        "mikai233",
        build_cluster_setting("127.0.0.1:12123".parse()?, client.clone())?,
    )?;
    ...
    for i in 0..10 {
        system1.spawn(
            Props::new_with_ctx(|_| Ok(EmptyTestActor)),
            format!("test_actor_{}", i),
        )?;
    }
    let sel = system1.actor_selection(ActorSelectionPath::RelativePath(
        "/user/../user/test_actor_*".to_string(),
    ))?;
    ...
    sel.tell(DynMessage::user(TestMessage), ActorRef::no_sender());
    {
        ...
        info!("aaaaaaaaa{:?}", m);
        cluster.leave(Address::new(
            "tcp",
            "mikai233",
            Some("127.0.0.1:12123".parse()?),
        ));
    }
    system2.await?;
    Ok(())
