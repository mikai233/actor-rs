...
use kairo_cluster::member::{Member, MemberStatus};
use kairo_cluster::unique_address::UniqueAddress;
use kairo_core::actor::actor_selection::{ActorSelection, ActorSelectionPath};
use kairo_core::actor::context::{ActorContext, Context, ContextExt};
use kairo_core::actor::coordinated_shutdown::{
    CoordinatedShutdown, PHASE_CLUSTER_SHARDING_SHUTDOWN_REGION,
};
use kairo_core::actor::dead_letter_listener::DeadMessage;
use kairo_core::actor::props::{Props, PropsBuilder};
...
use kairo_core::actor_ref::{ActorRef, ActorRefExt};
use kairo_core::ext::option_ext::OptionExt;
use kairo_core::message::message_buffer::{BufferEnvelope, MessageBufferMap};
use kairo_core::message::poison_pill::PoisonPill;
...
impl ShardRegion {
    ...
    fn registration_message(&self, context: &mut ActorContext) -> DynMessage {
        ...
        if self.entity_props.is_some() {
            DynMessage::user(Register {
                shard_region: myself,
            })
        } else {
            DynMessage::user(RegisterProxy {
    ...
    fn buffer_message(
        ...
        if total_buf_size >= buffer_size {
            ...
            context
                .system()
                .dead_letters()
                .cast_ns(DeadMessage(msg.into_dyn()));
        } else {
    ...
    fn get_shard(
        ...
        if self.starting_shards.contains(&id) {
            Ok(None)
        } else if let Some(shard) = self.shards.get(&id) {
            Ok(Some(shard.clone()))
        } else {
    ...
    fn send_graceful_shutdown_to_coordinator_if_in_progress(
        ...
        if self.graceful_shutdown_in_progress {
            ...
            for selection in actor_selections {
                selection.tell(
                    GracefulShutdownReq {
                        shard_region: context.myself().clone(),
                    }
                    .into_dyn(),
                    ActorRef::no_sender(),
                );
