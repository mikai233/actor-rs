...
impl State {
    pub(super) fn updated(&mut self, update: ShardState) {
        match update {
            ShardState::ShardHomeDeallocated { shard } => {
                ...
                for shards in self.regions.values_mut() {
                    shards.remove(&shard);
                }
            }
            ShardState::ShardRegionProxyTerminated { region_proxy } => {
                debug_assert!(
                    self.region_proxies.contains(&region_proxy),
                    "Terminated region proxy {} not registered: {:?}",
                    region_proxy,
                    self
                );
            ...
            ShardState::ShardRegionTerminated { region } => {
                debug_assert!(
                    ...
                    "Terminated region {} not registered: {:?}",
                    region,
                    self
                );
            ...
            ShardState::ShardRegionProxyRegistered { region_proxy } => {
                debug_assert!(
                    !self.region_proxies.contains(&region_proxy),
                    "Region proxy {} already registered: {:?}",
                    region_proxy,
                    self
                );
    ...
    pub(super) fn bin_state(&self) -> BinState {
        let shards: HashMap<String, ActorRef> = self
            .shards
            .iter()
            .map(|(shard_id, region)| (shard_id.clone().into(), region.clone()))
            .collect();
        let regions: HashMap<ActorRef, HashSet<String>> = self
            .regions
            .iter()
            .map(|(region, shards)| {
                (
                    region.clone(),
                    shards.iter().map(|shard| shard.clone().into()).collect(),
                )
            })
            .collect();
...
impl Display for State {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        let shards = self
            .shards
            .iter()
            .map(|(shard_id, region)| format!("<{},{}>", shard_id, region))
            .join(", ");
        let regions = self
            .regions
            .iter()
            .map(|(region, shards)| format!("<{},{}>", region, shards.iter().join(", ")))
            .join(", ");
        let region_proxies = self
            .region_proxies
            .iter()
        ...
        write!(
            f,
            "State {{ shards: {}, regions: {}, region_proxies: {} }}",
            shards, regions, region_proxies
        )
    }
}
...
impl From<BinState> for State {
    fn from(value: BinState) -> Self {
        let shards: HashMap<ImShardId, ActorRef> = value
            .shards
            .into_iter()
            .map(|(shard_id, region)| (shard_id.into(), region))
            .collect();
        let regions: HashMap<ActorRef, HashSet<ImShardId>> = value
            .regions
            .into_iter()
            .map(|(region, shards)| {
                (
                    region,
                    shards.into_iter().map(|shard| shard.into()).collect(),
                )
            })
            .collect();
        Self {
